...
e.g. configItems$CRAWL_NAME.cfg for ARCS in ./scripts/InternetArchive/crawlConfigFiles/configItems$CRAWL_NAME.cfg

1.1. Store config file for later access (e.g. on git)
	git add ./scripts/InternetArchive/crawlConfigFiles/configItems$CRAWL_NAME.cfg
	git commit -m "new config files for AWP15, AWP17 and AWP18"
	git remote set-url origin https://dcgomes@github.com/arquivo/scripts.git
	git remote add origin https://github.com/arquivo/scripts.git
	git push
...
e.g. #cp /shareP19/IAExchange/.bashrc ~
...
e.g. #./scripts/InternetArchive/uploadItems.sh ./scripts/InternetArchive/crawlConfigFiles/configItems$CRAWL_NAME.cfg 2>&1 1>uploadItems$CRAWL_NAME.out &
...
change config file nano #./scripts/InternetArchive/crawlConfigFiles/configItems$CRAWL_NAME.cfg OUTPUTFILE=/shareP19/scripts/IAExchange/itemsFor$CRAWL_NAME.recover

clean logs 
#mkdir ./finished/uploadLogs$CRAWL_NAME'-'$(date +"%d-%m-%Y"); mv ./*$CRAWL_NAME* ./finished/uploadLogs$CRAWL_NAME'-'$(date +"%d-%m-%Y")
...
4.1. Update documentation on Google Sheet (https://docs.google.com/spreadsheets/d/1SjijGAMXgUcwBaH1h7Pov9OLx2YpvR2G39R_CS1B9gs/edit#gid=1483363922; column "Replica on Internet Archive"). Insert link to IA query with custom field "pwacrawlid:$x_archive_meta_pwacrawlid" that returns all items of the crawl through https://archive.org/advancedsearch.php.
e.g. "=HYPERLINK("https://archive.org/advancedsearch.php?q=pwacrawlid%3AAWP21&fl%5B%5D=identifier&sort%5B%5D=&sort%5B%5D=&sort%5B%5D=&rows=50&page=1&callback=callback&save=yes&output=tables";"List of the 754 Items that compose AWP21 crawl. Query custom field: ""pwacrawlid:AWP21""")"
